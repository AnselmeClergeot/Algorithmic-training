...
Heap::Heap() : m_heap_array {}
{}

Heap::Heap(const std::initializer_list<int> elements) : m_heap_array(elements)
{
}

Heap::Heap(const std::vector<int> &elements) : m_heap_array(elements)
{
...
void Heap::build()
{
    for(int i {m_heap_array.size() - 1}; i >= 0; i--)
    {
        percolate_down(i);
    }
}
...
void Heap::insert_value(const int value)
{
    ...
    percolate_up(m_heap_array.size()-1);
}
...
{
    assert(lhs_index < m_heap_array.size() && rhs_index < m_heap_array.size());
...
{
    assert(!m_heap_array.empty());

    if(m_heap_array.size() > 1)
        swap(m_heap_array.size()-1, 0);
